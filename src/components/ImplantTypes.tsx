...
import { Anchor, Link2, Grid3x3, LayoutGrid } from "lucide-react";
    ...
    <section id="implant-types" className="scroll-mt-24 relative overflow-hidden bg-ink py-24 sm:py-28">
      ...
      <div className="container-x">
        ...
        <div className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {IMPLANT_TYPES.map(({ icon: Icon, title, body, points }, i) => (
            <Reveal key={title} delay={i * 0.08}>
              <article className="glass card-hover h-full rounded-3xl p-6">
                <div className="relative flex aspect-square items-center justify-center overflow-hidden rounded-2xl border border-gold-500/15 bg-[radial-gradient(circle_at_50%_30%,rgba(201,154,46,0.16),transparent_70%)]">
                  <Icon className="h-12 w-12 text-gold-300" strokeWidth={1.4} />
                </div>
